...
const Drawer: React.FC<HTMLAttributes<HTMLDivElement>> = memo(({ ...rest }) => {
  ...
  return (
    <aside
      className={clsx(
        "w-64 max-w-full h-screen flex fixed top-0 transition-transform z-20 flex-grow-0 flex-shrink-0",
    ...
    >
      <div className="w-full h-full flex flex-col relative">
        <div
          ...
          className="flex-shrink-0 basis-14 sticky flex justify-end bg-base-100 lg:bg-stone-200/50"
        >
          {!isLargeScreen && (
            <button
              onClick={closeDrawer}
            ...
            >
              <XMarkIcon className="size-9 rounded-full p-1 hover:bg-orange-500/10  transition " />
            </button>
          )}
        </div>
        ...
        <nav
          ...
          className="flex-grow w-full select-none shadow-lg bg-base-100"
        >
          {menuItems.map((item) => (
